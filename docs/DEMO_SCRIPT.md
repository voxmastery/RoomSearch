...
Film the browser at 1440×900 or wider, dark UI, one take if you can. The shot that matters is the latency pill on the Moss step. Do that shot with real keys. `DEMO_MOCK_MOSS=1` is for rehearsal only — the pill will say `Mock`, and that must not be the Moss shot.
